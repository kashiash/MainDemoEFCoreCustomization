...
9. **JS jako moduł ESM z idempotentnym `ensureRegistered()`, ładowany przez controller**. Powód: pozbywamy się hardcoded `<script>` w `_Host.cshtml` (kolejność ładowania bywa zawodna — wystarczy, że ktoś dorzuci kolejny `<script>` przed naszym i przestaje działać) i mamy gwarancję, że listener nie jest rejestrowany dwa razy nawet przy SignalR-reconnect.
